...
#include "aht20_test.c"  // 修改包含的头文件
#include "aht20.h"
#include "pwm_demo.h"
#include "mqtt_demo.h"

#define ADDRESS "tcp://e7790e6ea9.st1.iotda-device.cn-north-4.myhuaweicloud.com"
#define CLIENTID "6866264432771f177b48580a_18650_0_0_2025070807"
#define TOPIC "$oc/devices/6866264432771f177b48580a_18650/sys/properties/report"
#define PAYLOAD "Hello World!"
#define QOS 1

#define CONFIG_WIFI_SSID "test"       // 要连接的WiFi 热点账号
#define CONFIG_WIFI_PWD "gz216216" // 要连接的WiFi 热点密码
...
char *g_username = "6866264432771f177b48580a_18650"; // deviceId or nodeId
char *g_password = "44dcd262e855d1d537e99eb2637a444609259c11a2ecba52fcd33d707c762caa";
MQTTClient client;
extern int MQTTClient_init(void);

// 外部声明全局变量
extern float current_temp;
extern float current_humi;
...
void environment_task_entry(void)
{
    ...
    main_task(NULL); // 调用 aht20_test-1.c 中的初始化函数

    while (1) {
        mqtt_msg->msg_type = EN_MSG_REPORT;
        if ((mqtt_msg != NULL) && (current_temp != 0) && (current_humi != 0)) {
            sprintf(mqtt_msg->temp, "%.2f", current_temp);
            sprintf(mqtt_msg->humi, "%.2f", current_humi);
